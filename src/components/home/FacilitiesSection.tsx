  ...
  return (
    <section className="py-8 xs:py-10 sm:py-16 md:py-20 bg-background" ref={ref}>
      <div className="container mx-auto px-3 xs:px-4">
        ...
        >
          <span className="inline-block px-2.5 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 rounded-full bg-accent/10 text-accent text-[10px] xs:text-xs sm:text-sm font-medium mb-2 xs:mb-3 sm:mb-4">
            Our Facilities
          </span>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-3 xs:gap-4 sm:gap-6 md:gap-8">
          {facilities.map((facility, index) => (
            <motion.div
              key={facility.id}
              className="group relative rounded-xl sm:rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover active:scale-[0.98] transition-all duration-500 touch-manipulation"
              initial={{ opacity: 0, y: 30 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: index * 0.15 }}
            >
              {/* Image */}
              <div className="relative h-44 xs:h-48 sm:h-64 md:h-72 overflow-hidden">
                <img
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 xs:p-4 sm:p-6">
                <h3 className="font-display text-base xs:text-lg sm:text-xl font-semibold text-card mb-0.5 xs:mb-1 sm:mb-2">
                  {facility.title}
                </h3>
                <p className="text-card/80 text-[10px] xs:text-xs sm:text-sm leading-relaxed line-clamp-2">
